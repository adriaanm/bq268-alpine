...
- [ ] **QMI-proxy boot-time orphan subscriptions + modem restart reboots** — `qmi-proxy` as persistent multiplexer halved leak rate, but 4 `kworker/u8:N` still pile up in `D` state after every boot and `send_filled_buffers_to_user: Send Failed -3 drop_count=...` grows at ~10/min, pinning load average around 4. Suspect: modem init retry loop hammers QMI while Q6 DSP isn't ready. Fix: wait for Q6 readiness via sysfs state before issuing QMI.
  ...
  - **2026-07-05 session 2:** Q6→audio hypothesis **disproven** — Q6 healthy after the fix, stream clean full-duration, still silent. Real findings: speaker is on **EAR PA** (not HPHR→gpio36 ext amp as documented); analog stage alive (clicks measurable via Yeti + device mic); digital RX data dies in LPAIF→PDM→codec hop; `Playback 0 Volume` only applies to an open substream. Full log: `~/g/bq268/audio_experiments.md`. **Next: stock-Android A/B boot** to capture LPASS CSR 0x7702000/4/8 + codec regs during working playback.

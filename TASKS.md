...
- [ ] **LTE attach reliability on Swiss networks** — separate from the robustness work above: in testing on 2026-04-13 the modem could see all 5 LTE networks via `nas-network-scan=lte` (Swisscom, Salt, Sunrise, Free, 208-16) but failed to register to any of them automatically, whether on `lte,automatic` or even `lte,manual=22801`. Registration briefly transitions through `not-registered-searching` and then settles back to `not-registered` / `power-save`. SIM is ready, QMI prefs are correct, coverage exists. Hypothesis: roaming agreements at the current location do not permit Singtel SIM PS attach on any of the visible LTE networks right now — verify by testing at a different location before assuming a firmware bug. If the problem persists everywhere, investigate with DIAG logs or try a local SIM.

...
- [ ] eSIM provisioning — General APDU restriction patched (4 bytes in modem.b12, bit 2 bitmask). Non-ISD-R logical channels open. **ISD-R AID specifically blocked** by a separate modem filter (returns AccessDenied even with patch applied). Next: patch ISD-R AID filter OR use DIAG MMGSDI raw APDU. DIAG subsystems now respond (kernel #52 fix). See `docs/modem_patch_plan.md`.

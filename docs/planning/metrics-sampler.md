...
- **2026-04-13**: Scaffold committed (`tools/wata-metricsd/`). `protocol.zig` with `Tick` (16-byte packed struct, little-endian encode/decode), `sources.zig` with `Sample` + `Sources.sample()` reading battery voltage/current/capacity, wlan + rmnet operstate and rx/tx byte counters, and backlight brightness, and `jsonl.zig` with a pure `format(out, Record)` that serializes one JSONL line into a caller-provided buffer. Native and `arm-linux-musleabihf` ReleaseSafe builds both green, 10 unit tests passing. `main.zig` is still a stub. **Not yet implemented**: `batt_status` string reader, backlight auto-discovery, file-level sink (open/append + rotation), event loop, OpenRC service, `build-rootfs.sh` wiring.

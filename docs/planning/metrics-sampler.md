...
- **Sink write-failure handling**: per the *Error handling* spec, `main` now catches `sink.write` errors, writes a single `wata-metricsd: sink.write failed, continuing\n` line to stderr (which OpenRC captures to syslog), and latches a `sink_warned` flag so it stays silent until the next successful write. This gives us one syslog line per eMMC blip instead of either a crash or a log flood. Tests and the ARM ReleaseSmall build are both still green.

...
- **2026-04-13**: Scaffold committed (`tools/wata-metricsd/`). `protocol.zig` with `Tick` (16-byte packed struct, little-endian encode/decode), `sources.zig` with `Sample` + `Sources.sample()` reading battery voltage/current/capacity, wlan + rmnet operstate and rx/tx byte counters, and backlight brightness, `jsonl.zig` with a pure `format(out, Record)` that serializes one JSONL line into a caller-provided buffer, and `sink.zig` with a `Sink` that opens/appends `current.jsonl`, rotates on startup and on size threshold (`keep` files, configurable max bytes), all via raw `std.posix` + `std.os.linux` syscalls. Native and `arm-linux-musleabihf` ReleaseSafe builds both green, 12 unit tests passing (including two filesystem-touching tests under `/tmp`). `main.zig` is still a stub. **Not yet implemented**: `batt_status` string reader, backlight auto-discovery, event loop (SOCK_DGRAM bind + timerfd + poll + drain + sample + write), OpenRC service, `build-rootfs.sh` wiring.
